...
function checkExamAvailability(exam) {
  ...
  if (!exam.startTime || !exam.endTime)
    return { canStart: true, reason: "Available", color: "green" };
  ...
  if (now < start) {
    ...
    return {
      canStart: false,
      reason: mins < 60 ? `Starts in ${mins}m` : `Starts in ${Math.ceil(mins / 60)}h`,
      color: "orange",
    };
  }
  
  if (now > end) {
    return { canStart: false, reason: "Ended", color: "red" };
  }

  return {
    canStart: true,
    reason: `${Math.ceil((end - now) / 60000)}m left`,
    color: "green",
  };
}
  ...
  return (
    <div>
      <div className="mb-4 sm:mb-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-white">Available Exams</h2>
        <p className="text-gray-400 text-xs sm:text-sm mt-1">
          {exams.length} exam{exams.length !== 1 ? "s" : ""} listed
        </p>
      </div>
            ...
            <NeuCard key={exam._id}>
              <div className="flex justify-between items-start mb-3 gap-2">
                <div className="min-w-0">
                  ...
                  <p className="text-xs sm:text-sm text-gray-400">{exam.subject || "No subject"}</p>
                </div>
                {exam.resultReleased && (
                  <span className="text-xs px-2 py-1 rounded-full bg-green-500/20 border border-green-500/40 text-green-400 shrink-0">
                    ✓ Results Out
                  </span>
                )}
              </div>
              ...
              <p className="text-xs sm:text-sm text-gray-300 mb-3">
                ⏱ {exam.duration} mins
                {exam.totalMarks
                  ? <span className="ml-3 sm:ml-4">📊 {exam.totalMarks} marks</span>
                  : null}
              </p>

              {has && (
                <div className="mb-4 p-2.5 sm:p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl text-xs sm:text-sm">
              ...
              {av.canStart ? (
                <button
                  ...
                  className="mt-2 w-full px-4 py-2.5 bg-green-600 hover:bg-green-700 active:bg-green-800 text-white rounded-xl font-semibold text-sm transition touch-manipulation min-h-[44px]"
                >
                  Start Exam →
                </button>
              ) : (
                <button
                  disabled
                  className={`mt-2 w-full px-4 py-2.5 rounded-xl font-semibold text-sm cursor-not-allowed border min-h-[44px] ${STATUS_COLORS[av.color]}`}
                >
                  {av.reason}
                </button>
              )}
