...
export function generateAIInsights(detailed) {
  ...
  const stats   = detailed.statistics ?? detailed ?? {}; 
  
   
  const correct = stats.correctAnswers ?? 0;
  const wrong   = stats.incorrectAnswers ?? stats.wrongAnswers ?? 0;
  const skipped = stats.unanswered ?? stats.skippedQuestions ?? 0;
  ...
  if (timeMin !== null && timeMin < 10 && total > 20) 
