...
function StatCard({ title, value, color, icon }) {
  return (
    <div className="relative overflow-hidden rounded-2xl p-4 sm:p-5 bg-[#0f172a] border border-gray-800 hover:border-gray-600 transition">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <h2 className="text-2xl sm:text-3xl font-bold mt-1 text-white">{value}</h2>
        </div>
...
function InsightCard({ label, value, color }) {
  return (
    <div className="bg-[#1e293b] p-3 sm:p-4 rounded-lg border border-gray-700">
      <p className="text-gray-400 text-xs sm:text-sm">{label}</p>
      <p className={`font-semibold mt-1 text-sm sm:text-base ${color}`}>{value}</p>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ examsCreated: 0, activeExams: 0, pendingKeys: 0 });
  ...
  return (
    <div className="space-y-4 sm:space-y-6 text-white">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <h1 className="text-xl sm:text-2xl font-bold">Examiner Dashboard</h1>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={fetchStats}
            className="bg-blue-600 hover:bg-blue-700 active:bg-blue-800 px-3 sm:px-4 py-2 rounded-lg transition text-xs sm:text-sm touch-manipulation"
          >
            ↻ Refresh
          </button>
        </div>
      ...
      <NeuCard>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 text-sm">
          ...
          <InsightCard
            label="Exam Activity"
            value={stats.activeExams > 0 ? `${stats.activeExams} exams running` : "No active exams"}
            color="text-blue-400"
          />
